...
class VentaController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $productos = Producto::with('subCategoria')->where('disponible', 1)->get(); // Cargar la relación subCategoria
    ...
    public function generarVenta(Request $request)
    {
        ...
        foreach ($productos as $producto) {
            ...
            if ($productoModel->unidad_medida == 'KILOGRAMOS' || $productoModel->unidad_medida == 'LITROS') {
            ...
            $precio = $productoModel->precio_venta * $request->dollar;
            $totalNeto += $productoModel->precio * $cantidad;
         
            if ($productoModel->aplica_iva == 1) {
               
                $montoTotal += $cantidad * $precio * 1.16;
                $impuestosTotal += ($precio * 0.16) * $cantidad;
            } else {
        ...
        foreach ($productos as $producto) {
            ...
            $detalleVenta->neto = $precio * $cantidad;
            $detalleVenta->impuesto = (
                $productoModel->aplica_iva == 1
                ? ($precio * 0.16) * ($productoModel->unidad_medida == 'KILOGRAMOS' || $productoModel->unidad_medida == 'LITROS' ? $cantidad / 1000 : $cantidad)
                : 0
            );
            ...
            $productoModel->cantidad -= $cantidad;
            $productoModel->save();
