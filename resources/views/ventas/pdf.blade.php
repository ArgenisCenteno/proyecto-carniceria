<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CABARA</title>
    <link rel="stylesheet" href="{{public_path('css/bootstrap.min.css')}}"
        integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
    <link rel="stylesheet" href="{{public_path('css/pdf.css')}}"
        integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
</head>

<body class="Solvencia">

    <div class="SolvenciaTabla">

        <header class="container-fluid my-0 border-2 round-top mb-4">
            <table class="table table-borderless p-0 m-0">
                <thead>
                    <tr class="m-0 ">
                        <td class="py-1 align-middle m-0 text-center">
                            <h4 class="bolt">CARNICERÍA CABARA</h4>
                        </td>
                        <td class="py-1 text-center encabezado">
                            <p class="mb-0 bold" style="font-size: 0.6em">REPÚBLICA BOLIVARIANA DE VENEZUELA</p>
                            <p class="mb-0 bold" style="font-size: 0.6em">MUNICIPIO EZEQUIEL ZAMORA,
                                ESTADO MONAGAS</p>
                            <p class="mb-0 bold" style="font-size: 0.6em">MERCADO MUNICIPAL DE PUNTA DE MATA
                            </p>

                        </td>

                    </tr>

                </thead>
            </table>
        </header>


        <div class="border-dark container-fluid border-left round border-right border-top border-bottom  border-1 my-0">
            <div class="my-1 py-1 border-2 primeratabla">
                <h6 class="mb-0 text-center bold text-white">DATOS DE VENTA</h6>
            </div>
            <table class="table table-borderless p-0 m-0">
                <tbody>
                    <tr class="">
                        <td class="text-start">
                            @php
                                $id = str_pad($venta->id, 6, "0", STR_PAD_LEFT);
                            @endphp
                            <p class="m-0"> <strong>Venta N°: {{$id}}</strong></p>
                        </td>
                        <td class="text-start">
                            <p class="m-0"> <strong>Tipo: {{$venta->pago->tipo}}</strong>
                            </p>
                        </td>
                        <td class="text-end">
                            <p class="m-0"> <strong>Fecha:</strong> {{$fechaVenta}} </p>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div><br>


        <div
            class="container-fluid round border-dark my-0 py-1 border-bottom border-left border-right border-primary border-top border-1">
            <div class="my-2 py-1 border-2 primeratabla">
                <h6 class="mb-0 text-center bold">DATOS DEL CLIENTE</h6>
            </div>

            <table class="table table-borderless my-0">
                <tbody>
                    <tr>
                        <td style="width: 50%">
                            <p class="mb-0"><strong>C.I / R.I.F:</strong>
                                {{$userArray['dni']}} </p>
                            <p class="mb-0"><strong>NOMBRE/RAZÓN SOCIAL:</strong> {{$userArray['name']}} </p>

                        </td>
                        <td>
                            <p class="mb-0"><strong>DIRECCIÓN:</strong>
                                {{ $userArray['sector'] ?? 'Sector no disponible' }},
                                {{ $userArray['calle'] ?? 'Calle no disponible' }},
                                {{ $userArray['casa'] ?? 'Casa no disponible' }}
                            </p>

                            <p class="mb-0"><strong>CORREO ELECTRÓNICO:</strong>
                                {{ $userArray['email'] ?? 'Correo no disponible' }}
                            </p>

                        </td>
                    </tr>
                </tbody>
            </table>
        </div><br>



        <div
            class="container-fluid round-top border-dark my-0 py-1  border-left border-right border-primary border-top border-1">
            <div class="my-2 py-1 border-2 primeratabla">
                <h6 class="mb-1 text-center bold">PRODUCTOS ADQUIRIDOS</h6>
            </div>

            {{-- <p class="mb-0 p-small text-start bold">Pago por concepto de:

            </p>




            <p class="mb-0 p-small text-start bold">Observaciones:

            </p>

            --}}


        </div>

        <div
            class="container-fluid round-bottom border-dark my-0 py-1  border-left border-right border-primary border-bottom border-1">
            <table class="table table-bordered text-center my-1">
                <tbody>
                    <tr class="border" style="font-size:0.7em">
                        <th class="align-middle">Producto</th>
                        <th class="align-middle">Precio</th>
                        <th class="align-middle">Cantidad</th>
                        <th class="align-middle">Unidad de Medida</th>
                        <th class="align-middle">Neto</th>
                        <th class="align-middle">Impuesto</th>
                        <th class="align-middle">Subtotal</th>
                    </tr>

                    @foreach ($venta->detalleVentas as $detalle)
                                        <tr class="border">
                                            <td class="m-0 p-0 align-middle">
                                                @php 
                                                $unidad = null;
                                                                               if ($detalle->producto->unidad_medida == 'KILOGRAMOS' && $detalle->cantidad < 1) {
                                                                                $unidad = 'GRAMOS';
                                                    } elseif ($detalle->producto->unidad_medida == 'KILOGRAMOS' && $detalle->cantidad > 1) {
                                                        $unidad = 'KILOGRAMOS';
                                                    } elseif ($detalle->producto->unidad_medida == 'LITROS' && $detalle->cantidad < 1) {
                                                        $unidad = 'LITROS';
                                                    } elseif ($detalle->producto->unidad_medida == 'LITROS' && $detalle->cantidad > 1) {
                                                        $unidad = 'MILILITROS';
                                                    }

                                                  @endphp
                                                <p class="mb-0">{{ $detalle->producto->nombre }}</p>
                                                <!-- Asegúrate de que 'nombre' es el campo correcto -->
                                            </td>

                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{ number_format($detalle->precio_producto, 2) }} </p>
                                            </td>

                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{$unidad ? $detalle->cantidad * 1000 : $detalle->cantidad }} </p>
                                            </td>
                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{ $unidad ? $unidad : $detalle->producto->unidad_medida }} </p>
                                            </td>
                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{ number_format($detalle->neto, 2) }}</p>
                                            </td>

                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{ number_format($detalle->impuesto, 2) }}</p>
                                            </td>

                                            <td class="m-0 p-0 align-middle">
                                                <p class="mb-0">{{ number_format($detalle->neto + $detalle->impuesto, 2) }}</p>
                                            </td>
                                        </tr>
                    @endforeach

                </tbody>
            </table>

        </div><br>



        <div
            class="container-fluid round border-dark my-0 py-1 border-bottom border-left border-right border-primary border-top border-1">
            <div class="my-1 py-1 border-2 primeratabla">
                <h6 class="mb-0 text-center bold">Forma de Pago</h6>
            </div>



            <table class="table table-bordered text-center my-1">
                <thead>
                    <tr class="border" style="font-size:0.7em">
                        <th class="align-middle text-center">Forma de Pago</th>
                        <th class="align-middle text-center">Moneda</th>
                        <th class="align-middle text-center">Total Pagado</th>
                    </tr>
                </thead>
                <tbody>
                @foreach ($formaPagoArray as $valor)
                @if($valor['monto'] > 0)
                    <tr>
                       
                        <td  class="m-0 p-0 align-middle"> <p>{{$valor['metodo']  }}</p> </td>
                        <td class="m-0 p-0 align-middle"> <p> {{ $valor['metodo'] == 'Divisa' ? 'Dollar' : 'BOLÍVAR' }}</p>  </td>
                        <td class="m-0 p-0 align-middle"> <p> {{$valor['monto']  }}</p> </td>
                     
                       
                      
                    </tr>
                    @endif
                    @endforeach
                    <tr  >
                        <td class="m-0 p-0 align-middle fw-bold"> <p>Total</p>  </td>
                        <td class="m-0 p-0 align-middle"></td>
                        <td class="m-0 p-0 align-middle"> <p>{{ $venta->monto_total }}</p> </td>
                    </tr>
                </tbody>                                    
            </table>



        </div>
        <table class="table table-borderless text-center my-2">
            <tbody class="p-0 mt-5">
                <tr class="">
                    <th>

                    </th>
                    <th class="">
                        <!-- <p class="m-0 p-0 mt-4 border-top">Sello</p> -->
                    </th>
                    <th class="" style="width: 200px;">




                        <p class="m-0 p-0 mt-4 border-2 border-top">Vendedor</p>

                        <p class="m-0 p-0 mt-4 ">{{$vendedorArray['name']}}</p>


                    </th>

                </tr>

            </tbody>
        </table>


        <div class="title m-b-md">


        </div>




        {{-- <footer class="bg-success">
            <p class="text-centser">
                <?php echo date("d-m-y"); ?>
            </p>
        </footer> --}}
        <script type="text/php">
        if ( isset($pdf) ) {
            $pdf->page_script('
                $font = $fontMetrics->get_font("Arial, Helvetica, sans-serif", "normal");
                $pdf->text(500, 810, "Página $PAGE_NUM de $PAGE_COUNT", $font, 10);
            ');
        }
        </script>
    </div>
</body>

</html>